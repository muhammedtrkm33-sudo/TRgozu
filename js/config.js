...
const CONFIG = {
    ...
    // SOS: Kandilli ile sismik doğrulama (normal SOS yetkiliye yalnızca eşleşmede)
    ...
    // NASA FIRMS sıcak noktalar (FIRMS ile uyumlu NRT; yakın zamanda uydu geçişi)
    ...
    HEALTH_STATUS: {
        ...
        yaraliyim: { icon: '⚠️', label: 'Yaralıyım', color: '#ffa502' },
        kritik: { icon: '🚨', label: 'Kritik', color: '#ff4757' },
        enkaz: { icon: '🏗️', label: 'Enkaz Altında', color: '#9b59b6' }
    },
...
const STATE = {
    ...
    nearestGendarmerie: null,
    nearestMilitary: null,
    nearestGovernment: null,
...
window.addEventListener('error', function(event) {
    // Hataları sadece konsola yaz, ekranda sarı kutu gösterme
...
// Toast doğrudan göster (fallback), showToast tanımlanmadığında
function showToastDirect(message) {
    try {
        ...
        if (toast && toastMsg) {
            ...
            setTimeout(() => {
                try {
                    ...
                    console.error('Toast hide error:', e);
                }
            }, 5000);
        } else {
            console.warn('Toast element bulunamadı:', { toast: !!toast, toastMsg: !!toastMsg });
        }
    } catch (e) {
        console.error('showToastDirect error:', e);
    }
}
